...
.. list-table::
   ...
   * - API Dockerfile
     - ⬜ To do
     - Dependencies, entrypoint, health check
   ...
   * - Filesystem Collector
     - ✅ Done
     - Folder walk, supported file types, metadata
   * - PDF parsing & error handling
     - ⬜ To do
     - Ensure ingestion does not crash
   * - Chunking Module
     - ✅ Done
     - Paragraph splitting, chunk IDs
   * - Chunk size & overlap
     - ⬜ To do
     - 500–700 tokens, 10–20% overlap
   * - Preserve document context
     - ⬜ To do
     - Maintain references across chunks
   * - Embedding Module
     - ✅ Done
     - SentenceTransformers integrated, model loaded
   * - Batch processing / edge cases
     - ⬜ To do
     - Empty chunks, deterministic embeddings
   ...
   * - Storage Layer – Metadata DB
     - ✅ Done
     - SQLite tables, basic queries
   * - Query optimization
     - ⬜ To do
     - Indexing if needed
   * - API Layer
     - ✅ Done
     - FastAPI skeleton, some endpoints defined
   * - Wire ingestion/search pipelines
     - ⬜ To do
     - Connect collector → chunker → embed → DB
   * - Metadata filtering / Swagger
     - ⬜ To do
     - Complete API functionality
   * - CLI Client
     - ✅ Done
     - Typer skeleton
   * - Implement `ingest` / `search`
     - ⬜ To do
     - Connect to API, output formatting
   * - Error handling
     - ⬜ To do
     - Handle API failures
   * - Integration & Validation
