...
from storage.metadata_db.indexing_runs import load_latest_run_by_scope, create_run, update_status, load_latest_interrupted
...
def test_ingestion_service_is_resumable(tmp_path):
    ...
    # First ingestion
    result1 = ingest_scope(scope, run.id, resume=False)
    
    assert result1.documents_discovered == 1
    assert result1.documents_indexed == 1

    # Mark the run as "interrupted" for testing purposes
    update_status(run.id, "interrupted")

    # Laod interrupted run
    run2 = load_latest_interrupted("filesystem")
    
    # Second ingestion (resume)
    result2 = ingest_scope(scope, run2.id, resume=True)
