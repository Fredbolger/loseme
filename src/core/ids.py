...
def make_logical_document_part_id(
        source_instance_id: str, unit_locator: str
)-> str:
        """
        Create a stable logical document ID based on the source of the document and its unit locator (e.g., file path, email message ID, etc.).
        """
        text = f"{source_instance_id}:{unit_locator}"
...
def make_chunk_id(
    ...
    name = f"{document_part_id}:{document_checksum}:{index}"
    return hashlib.sha256(name.encode("utf-8")).hexdigest()

def make_thunderbird_source_id(
    # We require a unique identifier for the Thunderbird source instance, which can be derived from the device ID and the mbox path **without** reading the actual message content
    device_id: str,
    mbox_path: str,
    message_id: str,
) -> str:
