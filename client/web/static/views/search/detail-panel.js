...
// Slide-in Document Detail Panel + side-by-side Chunks Drawer
// ============================================================
...
let overlay = null;
let group = null;     // sliding unit: [chunksPanel, panel]
let panel = null;
let chunksPanel = null;
...
function createPanel() {
  ...
  // Sliding group — chunks drawer first (left), detail panel second (right)
  group = document.createElement('div');
  group.className = 'detail-panel-group';

  chunksPanel = document.createElement('div');
  chunksPanel.className = 'chunks-preview-panel';
...
function toggleChunksPreview(forceState) {
  ...
  chunksPanel.classList.toggle('open', chunksPreviewOpen);

  // Extend the main-content shift so it keeps clearing the wider group
  const searchMain = document.querySelector('.search-main');
  const sourcesMain = document.querySelector('.sources-main');
  if (searchMain) searchMain.classList.toggle('chunks-panel-open', chunksPreviewOpen);
  if (sourcesMain) sourcesMain.classList.toggle('chunks-panel-open', chunksPreviewOpen);
...
export function openDetail(docId, sources, sourceType, sourcePath) {
  ...
  if (searchMain) searchMain.classList.add('detail-panel-open');
  if (sourcesMain) sourcesMain.classList.add('detail-panel-open');
...
export function closeDetail() {
  ...
  // Collapse the chunks drawer too, so re-opening starts clean
  if (chunksPanel) chunksPanel.classList.remove('open');
  ...
  if (searchMain) {
    ...
    searchMain.classList.remove('chunks-panel-open');
  }
  if (sourcesMain) {
    sourcesMain.classList.remove('detail-panel-open');
    sourcesMain.classList.remove('chunks-panel-open');
  }
...
async function loadDetail(docId, sourceType, sourcePath) {
  ...
  try {
    ...
    // Keep the drawer's content in sync if it's already open while navigating
    if (chunksPreviewOpen) {
      loadChunksForDocument(docId);
    }
  } catch (e) {
    body.innerHTML = `
